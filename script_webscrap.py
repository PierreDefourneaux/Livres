...
print("début du scraping des 10 pages \"les livres en vogue cette semaine\"...")

for i in range(1,11):
    url_scrap = f"https://openlibrary.org/trending/weekly?page={i}"
               ...
               #exctraction des ISBN 13
                ...
                try:
                    ...
                    if auteur :
                        ...
                        a = auteurs.append(str(google_books_api(titre)[4]))
                        if a:
...
df_data_webscrap_a_nettoyer.to_csv(f'df_data_webscrap_a_nettoyer.csv', index=False)
...
print(f"""Le fichier csv "df_data_webscrap_a_nettoyer.csv" a bien été généré.""")
print("\nFin de l'exécution du script :",datetime.now().strftime("%Y-%m-%d %H:%M:%S"))